...
u24p.buildUI = function (id) { // build User Interface
    ...
    u24p.cases.forEach(function (c) {
        ...
        btFeature.onclick = function () {
            ...
            var url = 'http://' + config.domain + '/featurescape/?' + u24p.findApi + '?limit=' + sz + '&find={%22randval%22:{%22$gte%22:' + sp.textContent + '},' + _static + ',%22provenance.image.case_id%22:%22' + caseId + '%22}';
            if (config.mongoUrl)
            {
                url = url + '&mongoUrl=' + config.mongoUrl;
            }
            url = url + ';fun/u24demo.js';
            window.open(url);
