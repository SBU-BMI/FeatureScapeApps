...
openHealth.ini = function () {
    ...
    this.getJSON = function (url, fun, store) {
        if (!this.getJSON.cache) { // if caching not enabled
            if (!store) {
                store = openHealth.getJSON.store
            }
            this.xhr(url, function (x) {
                var y = JSON.parse(x.target.responseText);
                fun(y);
                if (store) {
                    localforage.setItem(encodeURIComponent(url), y);
                } // if store is true cache nonetheless
            });
        } else {
    ...
    this.sodaAll = function (url, q, fun, xx, fun0) { // version of soda2 that keeps reading reccords untill all are retrieved
        ...
        if (!xx) {
            xx = []
        } // the array being assembled throughout multiple calls
        if (!fun0) {
            fun0 = fun
        } // to carry original fun all eh way to the end
        var moreFun = function (x) {
            xx = xx.concat(x);
    ...
    this.txt2docs = function (txt) {
        if (txt[0] == '[') { // txt is json
            ...
            var y = txt.split(/[\n\r]/).map(function (xi) {
                return xi.split(/\t/).map(function (c) {
                    return c.replace(/["\,]/g, '')
                })
            });
            var docs = [];
    ...
    this.interp1 = function (X, Y, XI) { // linear interpolation, remember X is supposed to be sorted
        ...
        var YI = XI.map(function (XIi) {
            var i = openHealth.sum(X.map(function (Xi) {
                if (Xi < XIi) {
                    return 1
                } else {
                    return 0
            ...
            if (i == 0) {
                ...
                return (Y[i - 1] + (XIi - X[i - 1]) * (Y[i] - Y[i - 1]) / (X[i] - X[i - 1]))
            }
        });
        return YI
    };
    ...
    this.memb = function (x, dst) { // builds membership function
        ...
        if (!dst) {
            dst = this.sort(x);
            Ind = dst[1];
            dst[1] = dst[1].map(function (z, i) {
                return i / (n)
            });
            var y = x.map(function (z, i) {
                return dst[1][Ind[i]]
            });
            return dst;
        }
    ...
    this.tabulateCount = function (tab, p1, p2, Up1, Up2) {//tabulate parameter p1 against p2, optional: arrays with unique values
        if (Array.isArray(tab)) {
            tab = this.tab2docs(tab)
        } // in case an array of docs is being submitted
        if (!Up1) {
            Up1 = this.unique(tab[p1])
        }
        if (!Up2) {
            Up2 = this.unique(tab[p2])
        }
        var tabcount = {};
    ...
    this.crossdoc2html = function (d, title, sort) { // create table from cross-document
        ...
        for (j = 0; j < cols.length; j++) {
            html += '<th style="text-align:right">' + cols[j] + '</th>'
        }
        html += '</tr>';
        // body
        for (i = 0; i < rows.length; i++) {
            ...
            for (j = 1; j < cols.length; j++) {
                html += '<td style="text-align:right;color:blue">' + d[rows[i]][cols[j]] + '</td>'
            }
            html += '</tr>';
        }
        html += '</table>';
        ...
        sp.textContent = ' as CSV with filename: ';
        sp.style.color = "navy";
        var ipTitle = document.createElement('input');
        ipTitle.size = 100;
        ipTitle.style.color = "green";
        ipTitle.value = title;
        //lala = bt;
        bt.d = d;// the data
        bt.onclick = function () {
    ...
    this.npi = function (q, fun) { // retrieve data on a National Provider Identifier from CMS
        ...
        if (q.match(/^[\d]+$/)) {
            q = 'npi=' + q
        } // if the query is an integer then assume it is the NPI
        this.soda(url, q, fun);
        return this
    };
